...
import { R18_LOG_LIB, R18_DIALOGUES } from './r18.js';
...
for (let pName in R18_DIALOGUES) {
    // 如果主库里有这个性格
    if (NEW_DIALOGUE_LIB[pName]) {
        // 把 R18 库里的字段 (如 captor_r18, imprisoned_r18) 拷贝过去
        Object.assign(NEW_DIALOGUE_LIB[pName], R18_DIALOGUES[pName]);
    }
}
console.log("【系统】R18 对话库已成功注入 NEW_DIALOGUE_LIB");
    ...
    getSpecialDialogue: function(person, type) {
       ...
       if (person.id === gs.captorId && person.isPregnant) {
            ...
            if (type === "chat") {
                // --- A. 普通版文案 (沉重/宿命感) ---
                const normalTexts = [
                    "他抚摸着微隆的小腹，眼神疯魔又卑微：“这便你给我的报应吗？流淌着你的血液的孩子……我会把他生下来，他将是你我永恒的锁链。”",
                    "“你是我的……我会为你孕育后代，不论你愿不愿意”",
                    "他面色苍白，忍着腹中的阵痛，却还要强撑着冷笑：“看啊，连这天理伦常都在帮我留住你。”",
                    "他低头看着肚子，声音沙哑：“你说，他出生时是会像你多一点，还是像我这个囚禁你的罪人多一点？”"
                ];

                // --- B. R18 版文案
                const r18Texts = [
                    "他衣衫半敞，露出因怀孕而肿胀发亮的乳尖，乳头挺立着，顶端正缓缓渗出乳白液体。他走到你面前，锁链轻响，低头看着自己胸口，又抬头看你，声音又软又哑：“哈……自从怀了你的孩子，这身子就骚得不行……每天都流奶水，只想被你吸……”他抓着你的手按在自己乳尖上，让温热的乳汁沾湿你的指尖，眼神湿漉漉的：“现在你出不去了，只能看我……只能碰我……知道吗？”",

                     "“看啊……”他抓着你的手，强行按在他隆起的肚子上，让你清楚地感受到里面的胎动，同时下体已经硬得发紫，顶端不断渗出前液。“这孩子在里面动一下，我就爽得想高潮……这都是你害的。”他贴近你，嘴唇几乎擦到你耳廓，声音带着颤：“以后你再也出不去沾花惹草了……只能看着我、摸着我、被我操……永远只看我一个人，懂吗？”",

                     "他扶着后腰，挺着圆润的肚子，慢慢跨坐在你身上，锁链随着动作叮当作响，满脸潮红。“怀着孕让我奶子好涨……你饿了吗……”他把肿胀的乳尖送到你唇边，乳汁已经溢出来，滴在你唇上：“来吃我的奶吧……我想做你的母亲……从今往后，你只能吃我的奶、操我的身子……再也不许看别人了，好不好？”",

                     "“唔……涨得好痛……”他难耐地揉捏着自己溢乳的胸脯，乳白液体顺着手指往下淌，眼神又痛苦又淫荡。“都是你的错……把我变成这副只知道求欢的淫荡模样……”他忽然低下头，咬着你的耳朵，声音又软又狠：“既然是你干的好事，就来安抚我……现在你被我关在这里，只能被我操、吸我、让我射在你身体里……让你永远只属于我一个人。”"
                ];

                // 根据 R18 开关决定用哪一组
                let targetTexts = normalTexts;
                if (gs.settings && gs.settings.enableR18) {
                    targetTexts = r18Texts;
                }
                
                return targetTexts[Math.floor(Math.random() * targetTexts.length)];
            }
        }
        if (gs.settings && gs.settings.enableR18) {
            // 你被他囚禁 OR 他被你囚禁
            if ((gs.isPlayerImprisoned && person.id === gs.captorId) || person.isImprisoned) {
                return null; // 返回 null，让 actions.js 去调用通用的 getTalk (那里有丰富的 R18 文案)
            }
        }
...
// --- R18 囚禁交谈判定 ---
            if (gs.settings.enableR18) {
                // 1. 如果你是囚犯 (Captor R18)
                if (gs.isPlayerImprisoned && p.id === gs.captorId && lib.captor_r18) {
                    return randomChoice(lib.captor_r18);
                }
                // 2. 如果他是囚犯 (Imprisoned R18)
                if (p.isImprisoned && lib.imprisoned_r18) {
                    return randomChoice(lib.imprisoned_r18);
                }
            }
            // 入魔判定
            if (p.isDemonic) {
       ...
       getR18Log: function(p, specialType = null) {
    // 1. 找到该性格的 R18 库
    let lib = R18_LOG_LIB[p.personality.name] || R18_LOG_LIB["温润"];
    const gs = window.gameState;

    // --- 优先级 0：特殊状态强制覆盖 (如勒索、迷情香) ---
    // 1. 勒索 (新增)
        if (specialType === 'blackmail' && lib.blackmail) {
            return randomChoice(lib.blackmail);
        }
        //2.迷情香
    if (specialType === 'drug' && lib.drug) {
        return randomChoice(lib.drug);
    }
// --- 优先级 0.5：玩家心魔状态 (自动检测) ---
        // 如果玩家有心魔buff，且没有指定其他特殊类型(比如没下药)，优先触发心魔文案
        if (gs.player.buffs && gs.player.buffs.xin_mo_yu && lib.xin_mo) {
             return randomChoice(lib.xin_mo);
        }
    // --- 优先级 1：囚禁判定 ---
    // 如果 NPC 是囚禁者（他关你）
    if (gs.isPlayerImprisoned && p.id === gs.captorId) {
        return randomChoice(lib.captor || lib.high_dark);
    }
    // 如果 NPC 是被囚禁者（你关他）
    if (p.isImprisoned) {
        return randomChoice(lib.imprisoned || lib.high_dark);
    }

    // --- 优先级 2：场景判定 (野外) ---
    // 如果玩家在“十万大山(wild)”或其他危险区域
    if (gs.player.location === 'wild' && lib.wild) {
        return randomChoice(lib.wild);
    }

    // --- 优先级 3：黑化判定 ---
    // 2. 根据黑化值判断是“延伸版”还是“崩坏版”
    let isHighDark = (p.darkness >= 50); 
    let lines = isHighDark ? lib.high_dark : lib.low_dark;
...
// ==================== 【R18修复补丁开始】 ====================
// 这是一个自动运行的脚本，用于把 r18.js 里的内容“注射”进主文案库
console.log("【系统】正在尝试加载 R18 扩展文案...");

if (typeof R18_DIALOGUES !== 'undefined') {
    for (let pName in R18_DIALOGUES) {
        // 如果主库里有这个性格（比如“疏狂”、“骄阳”等）
        if (NEW_DIALOGUE_LIB[pName]) {
            // 将 R18 的字段（如 captor_r18）合并进去
            Object.assign(NEW_DIALOGUE_LIB[pName], R18_DIALOGUES[pName]);
            console.log("  > 已注入 R18 文案库: " + pName);
        }
    }
    // 【测试专用】把修改后的库挂载到 window 上，方便你在控制台检查
    window.TEST_LIB = NEW_DIALOGUE_LIB; 
    console.log("【系统】R18 文案加载完毕！请在控制台输入 window.TEST_LIB 查看。");
} else {
    console.error("【错误】未找到 R18_DIALOGUES 变量，请检查头部 import 是否包含它！");
}
// ==================== 【R18修复补丁结束】 ====================